...
def _api_response_from_http(response: httpx.Response) -> APIResponse:
    """HTTP 応答を APIResponse に変換する。

    Image API の成功時は body に message のみで code が無いことがある。
    その場合は HTTP ステータスを code として扱う。
    """
    try:
...
class Quote0Client:
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None):
        ...
        self._client = httpx.Client(trust_env=False, timeout=self.REQUEST_TIMEOUT_SECONDS)
    ...
    def switch_to_next(self, device_id: str) -> APIResponse:
        ...
        return _api_response_from_http(response)
